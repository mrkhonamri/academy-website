...
export default function AdminTestimonialsPage() {
  ...
  const [form, setForm] = useState({ name: "", role: "", text: "", stars: 5, isActive: true, isApproved: false, sortOrder: 0 });
  ...
  function resetForm() {
    setForm({ name: "", role: "", text: "", stars: 5, isActive: true, isApproved: false, sortOrder: 0 });
    setEditingId(null); setShowForm(false);
  }

  function editItem(t: any) {
    setForm({ name: t.name, role: t.role, text: t.text, stars: t.stars, isActive: t.isActive, isApproved: t.isApproved, sortOrder: t.sortOrder });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    resetForm(); await fetchItems(); router.refresh();
  }

  async function approveItem(id: number) {
    await fetch(`/api/testimonials/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isApproved: true }) });
    await fetchItems(); router.refresh();
  }

  async function rejectItem(id: number) {
    await fetch(`/api/testimonials/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isApproved: false }) });
    await fetchItems(); router.refresh();
  }
  ...
  return (
    <div className="mx-auto max-w-4xl p-6" dir="rtl">
        ...
        <form onSubmit={handleSubmit} className="mb-8 rounded-xl border bg-white p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="flex items-center gap-2 pt-6">
              <input type="checkbox" id="isApproved" checked={form.isApproved} onChange={e => setForm({...form, isApproved: e.target.checked})} className="h-4 w-4" />
              <label htmlFor="isApproved" className="text-sm font-medium">تایید شده</label>
            </div>
          </div>
      ...
      <div className="space-y-3">
        {items.map(t => (
          <div key={t.id} className={`flex items-center justify-between rounded-xl border p-4 ${t.isApproved ? "bg-white" : "bg-amber-50 border-amber-200"}`}>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold">{t.name}</h3>
                <span className="text-amber-400">{"⭐".repeat(t.stars)}</span>
                {!t.isApproved && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs text-amber-700">در انتظار تایید</span>}
              </div>
              <p className="text-sm text-slate-500">{t.text}</p>
            </div>
            <div className="flex items-center gap-2">
              {!t.isApproved ? (
                <button onClick={() => approveItem(t.id)} className="rounded-lg p-2 text-green-600 hover:bg-green-50"><Check className="h-4 w-4" /></button>
              ) : (
                <button onClick={() => rejectItem(t.id)} className="rounded-lg p-2 text-amber-600 hover:bg-amber-50"><X className="h-4 w-4" /></button>
              )}
